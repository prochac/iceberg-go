...
func TestSigningRoundTripper(t *testing.T) {
	...
	t.Run("signer error returns error", func(t *testing.T) {
		...
		rt := &signingRoundTripper{
			base:          http.DefaultTransport,
			signerURI:     signerServer.URL,
			tokenProvider: func() (string, error) { return "expired-token", nil },
			region:        "us-east-1",
		}
...
func TestSigningRoundTripperDynamicToken(t *testing.T) {
	t.Parallel()

	callCount := 0
	signerServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount++
		expectedToken := "token-" + string(rune('0'+callCount))
	...
	rt := &signingRoundTripper{
		...
		tokenProvider: func() (string, error) {
			...
			return "token-" + string(rune('0'+tokenSeq)), nil
		},
		region: "us-east-1",
	}
...
func TestSigningRoundTripperTokenProviderError(t *testing.T) {
	t.Parallel()

	rt := &signingRoundTripper{
		base:      http.DefaultTransport,
		signerURI: "http://localhost/sign",
		tokenProvider: func() (string, error) {
	...
	_, err = rt.RoundTrip(req)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to get signing token")
...
func TestSignerEndpoint(t *testing.T) {
	...
	tests := []struct {
		...
		{"default path", "https://catalog.example.com", "", "https://catalog.example.com/v1/aws/s3/sign"},
		{"default path with trailing slash", "https://catalog.example.com/", "", "https://catalog.example.com/v1/aws/s3/sign"},
		{"default path with base path", "https://catalog.example.com/api", "", "https://catalog.example.com/api/v1/aws/s3/sign"},
		{"custom endpoint", "https://catalog.example.com/catalog/", "v1/signer/warehouse-id/table-id/v1/aws/s3/sign", "https://catalog.example.com/catalog/v1/signer/warehouse-id/table-id/v1/aws/s3/sign"},
		{"custom endpoint with leading slash", "https://catalog.example.com", "/v1/custom/sign", "https://catalog.example.com/v1/custom/sign"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tt.expected, signerEndpoint(tt.uri, tt.endpoint))
		})
	}
}
